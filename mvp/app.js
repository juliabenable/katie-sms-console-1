...
const LS_KEY = 'katie-sms-console-v1';

function loadState() {
  try {
    const saved = JSON.parse(localStorage.getItem(LS_KEY));
    if (saved && saved.v === 1 && saved.gen === SEED.gen) return saved;
  } catch (e) {}
  return {
    v: 1,
    gen: SEED.gen,
...
function flagCardHTML(f, forDrawer) {
  ...
  return `<div class="fc-reason">Held: ${esc(f.reason)}</div>
    ...
    <div class="fc-draft" data-draft="${f.id}">${esc(f.draft)}</div>
...
function approveFlag(id) {
  ...
  const { t, ts } = nowStamp();
  S.messages.push({ id: 'a' + Date.now(), creatorId: f.creatorId, author: 'agent', senderName: '', line: f.line || 'main', body, t, ts, meta: 'approved by Katie' });
...
function renderLadders() {
  ...
  for (const r of S.ladders[activeLadder]) {
    ...
    line.innerHTML = `<span class="msgid">${esc(r.id)}</span><span class="l-name">${esc(r.name)}</span><span class="l-when">${esc(r.when)}</span>${r.edited ? '<span class="st editedchip">edited</span>' : ''}<span class="st ${r.status}">${r.status === 'new' ? 'new draft' : r.status}</span>`;
...
function renderAll() { persistOpenEdits(); renderFeed(); renderFlags(); renderLadders(); if (drawerCreator) renderDrawerBody(); }
